...
<body>
            <div class="actions">
                <div class="btn-group">
                    <a class="btn btn-default btn-sm"
                     href="<%=request.getContextPath()%>/prsArrgInfo/addpage"
                    >
	...
	<script type='text/javascript'>    
        ...
        function jumpOnEditPage(id){
        	if(!!id){
        		window.location.href=urlRootContext+"/prsArrgInfo/editPage/"+id;	
        	}        	
        }
        ...
        function deleEvent(id) {        	
        	if (confirm("确定要删除吗？")) {
	            ...
	            var postPath = strPath.substring(0, strPath.substr(1).indexOf('/') + 1);
                $.ajax({
                    url: postPath + "/prsArrgInfo/delete",
                    type: "POST",
                    async: false,// 同步请求
                    data:{"id":id},
                    success: function () {
        ...
        function buildTable(userName,pageNumber,pageSize) {
        	 var url =  urlRootContext + "/prsArrgInfo/findPrsArrgInfoList"; //请求的网址
         ...
         if (dataList.length > 0 ) {
             $(dataList).each(function(){//重新生成
                    ...
                    $("#tableBody").append('<td>' + this.applydate + '</td>');
                    $("#tableBody").append('<td>' + this.applypolice + '</td>');
                    $("#tableBody").append('<td>' + this.prsfullname + '</td>');
