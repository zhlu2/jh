        ...
        function jumpOnShowPage(id){
        	if(!!id){
        		window.location.href=urlRootContext+"/dedrugBasicInfo/showPage/"+id;	
        	}        	
        }
        ...
        function deleEvent(id) {        	
        	if (confirm("确定要删除吗？")) {
	            ...
	            var postPath = strPath.substring(0, strPath.substr(1).indexOf('/') + 1);
                $.ajax({
                    url: postPath + "/dedrugBasicInfo/delete",
                    type: "POST",
                    async: false,// 同步请求
                    data:{"id":id},
                    success: function () {
        ...
        function buildTable(userName,pageNumber,pageSize) {
        	 var url =  urlRootContext + "/dedrugBasicInfo/findDedrugBasicInfoList"; //请求的网址
         ...
         if (dataList.length > 0 ) {
             $(dataList).each(function(){//重新生成
                    ...
                    $("#tableBody").append("<a href=\"javascript:jumpOnShowPage("+this.id+");\" class=\"btn default btn-xs blue-stripe\">查看</a>");                  
                    $("#tableBody").append('</tr>');
             	    });  
             	    } else {             	            	
