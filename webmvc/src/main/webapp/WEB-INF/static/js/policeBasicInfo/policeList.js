        ...
        function jumpOnEditPage(id){
        	if(!!id){
        		window.location.href=urlRootContext+"/policeBasicInfo/editPage/"+id;	
        	}        	
        }
        ...
        function deleEvent(id) {        	
        	if (confirm("确定要删除吗？")) {
	            ...
	            var postPath = strPath.substring(0, strPath.substr(1).indexOf('/') + 1);
                $.ajax({
                    url: postPath + "/policeBasicInfo/delete",
                    type: "POST",
                    async: false,// 同步请求
                    data:{"id":id},
                    success: function () {
        ...
        function buildTable(userName,pageNumber,pageSize) {
         ...
         if (dataList.length > 0 ) {
             $(dataList).each(function(){//重新生成
                    ...
                    $("#tableBody").append('<td>' + this.policenum + '</td>');
                    $("#tableBody").append('<td>' + this.policename + '</td>');
                    $("#tableBody").append('<td>' + this.policesex + '</td>');
