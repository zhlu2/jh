        ...
        function jumpOnEditPage(id){
        	if(!!id){
        		window.location.href=urlRootContext+"/dedrugBasicInfo/editDeletePrison/"+id;	
        	}        	
        }

        //生成表格
        function buildTable(userName,pageNumber,pageSize) {
        	 var url =  urlRootContext + "/dedrugBasicInfo/selectByDelete"; //请求的网址
         ...
         if (dataList.length > 0 ) {
             $(dataList).each(function(){//重新生成
                    ...
                    $("#tableBody").append("<a href=\"javascript:jumpOnEditPage("+this.id+");\" class=\"btn default btn-xs blue-stripe\">办理</a>");                  
                    $("#tableBody").append('</tr>');
             	    });  
             	    } else {             	            	
